...
function RemoveBudget({ updateHomeData }) {
  ...
  const fetchDelocatedCategories = (year, month) => {
    ...
    axios
      .get(`http://144.202.18.160:3002/api/get-budgets/${year}/${month}/${userId}`)
      .then((response) => {
  ...
  const fetchAllCategories = async () => {
    try {
      const response = await axios.get('http://144.202.18.160:3002/api/categories');
  ...
  const handleDeallocationSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://144.202.18.160:3002/api/deallocation-budget', deallocateData);
